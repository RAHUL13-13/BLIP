    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_data_loader):
            ...
            if self.tokenizer is not None:
                data['text'] = self.tokenizer(text=data['text'], return_tensors='pt', padding=True,
                                              truncation=True)
            ...
            loss = self.loss(output, self.model.blip.blip.logit_scale)
            loss.backward()
            ...
            torch.clamp_(self.model.blip.blip.logit_scale.data, max=np.log(100))
    ...
    def _valid_epoch_step(self, epoch, step, num_steps):
        ...
        with torch.no_grad():
            for _, data in tqdm(enumerate(self.valid_data_loader)):
                ...
                curr_loss = self.loss(sims_batch, self.model.blip.blip.logit_scale)
            ...
            self.model.pool_frames.to(torch.device('cuda:2'))
